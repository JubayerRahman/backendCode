...
const tokenList = client.db("SGE_REG").collection("tokenList")
...
const dbConnect = async () => {
    ...
    app.post('/saveToken', async(req, res)=>{
        ...
        const result = tokenList.insertOne(data)
        res.send(result)
      })
  
      app.delete('/removeToken', async(req, res)=>{
        const {user} = req.body
        console.log(user);
        const filter = ({user: user})
        const result = await  tokenList.deleteMany(filter)
        res.send(result)
      })
  
      async function sendingNotification(expoToken){
        const message = {
          to: expoToken,
          sound: 'default',
    ...
    //   Notification functions Ends
        ...
        app.patch('/registrationPatchStatus/:_id', async (req, res) => {
            ...
            const cpMail = req.body.counsellorMail
            console.log(cpMail);
            const counsellor = await tokenList.findOne({user:cpMail})

            if (!counsellor) {
                console.log("No counsellor available");
                return
              }
              const pushToken = counsellor.expoToken
        
              await sendingNotification(pushToken)

            // notification related codes ends here
